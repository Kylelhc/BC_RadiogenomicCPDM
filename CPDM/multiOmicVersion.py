...
def p_sample_loop(model, shape, condition, timesteps):
    ...
    ###
    btf_embedding = torch.nn.Linear(17, shape[2]*shape[2], bias=False).to('cuda')
    btf_embedding.weight.requires_grad = False
...
def drawPDMloss(loss_history, epochs): # good
  ...
  sns.lineplot(x=[*range(1,epochs+1)],y=loss_hist["train"],label='Train loss',color='#28A745')
  sns.lineplot(x=[*range(1,epochs+1)],y=loss_hist["val"],label='Test loss',color='#FF6B6B')
...
def generateAllImages(model,timesteps,numGenerate=1,savePath=None,btfPath=None,interrupt=0,gridnum=4,figureSize=32): # good
  """ Generate All Images """
...
def calculate_inception_score(p_yx, eps=1E-3):
    min_value = np.min(p_yx)
    max_value = np.max(p_yx)
    p_yx = (p_yx - min_value) / (max_value - min_value)
...
torch.manual_seed(0)
btfs_train, btfs_test, tensors_train, tensors_test, status_train, status_test, clinicalStatus_train, clinicalStatus_test = prepareDataset_end2end(SideImgPath,SideBTFpath,allBTFpath,mutationStatusPath,clinicalDataPath,gene,clinicalStatus)
train_loader, val_loader = get_dataloaderM(batch_size, btfs_train, btfs_test, tensors_train, tensors_test)
...
if sys.argv[1] == 'train':
    ...
    if sys.argv[1] == 'unpaired':
        ...
        testIds = ['TCGA.AO.A03M', 'TCGA.AO.A12E', 'TCGA.AO.A12G', 'TCGA.AR.A1AX', ]
        for eid in testIds:
            print(eid)
            indx = testIds.index(eid)
            simgs = showUnpairedResults(model, timesteps, generatedSamp=3, btfpath=btfPath, testIds=[eid], gridnum=4,figureSize=32)
            all_fid = []
            real = tensors_test[indx].cpu().numpy()[0]
            for each in simgs[-1]:
                gener = each[0]
                all_fid.append(calculate_fid(real, gener))
            print('Best fid:', min(all_fid))
            print('Ave fid:', sum(all_fid) / 3)
            print('Standard deviation:', statistics.stdev(all_fid))
            print('IS:', calculate_inception_score(simgs[-1], eps=1E-16))
    elif sys.argv[1] == 'generate':
        generateAllImages(model,timesteps,numGenerate=1,savePath=savePath,btfPath=btfPath,interrupt=4000,gridnum=2,figureSize=5)
    else:
        print('Error mode')
        raise Exception
