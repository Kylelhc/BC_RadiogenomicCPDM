...
def prepareDataset(SideImgPath,SideBTFpath,allBTFpath,device, test = [1,16,24,33]):
  ...
  for bt in btf:
      ...
      df_empty=df_empty._append(label,ignore_index=True)
...
def train_pdm(config):
  ...
  multiOmicPath = config['multiOmicPath']
  test = config['test']
  ...
  btfs_train, btfs_test, tensors_train, tensors_test = prepareDataset(SideImgPath,SideImgIDpath,multiOmicPath,device,test)
